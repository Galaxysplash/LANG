...
void tree::get_numbers_and_head(
    ...
    const std::function call_func = [&func_in](int& count_for_calls_ref, const double& number_in, const std::string& str_buffer_in) {
        func_in(count_for_calls_ref, number_in, str_buffer_in);
        ++count_for_calls_ref;
    };
...
#define func_in FORBIDDEN
    for (int count_for_calls = 0; const std::string& potential_number_ref: instruction_in) {
        if (g_num_list.contains(potential_number_ref)) {
            call_func(count_for_calls, g_num_list[potential_number_ref], str_buffer);
        }
        else {
            try {
                call_func(count_for_calls, std::stod(potential_number_ref), str_buffer);
                str_buffer.clear();
            } catch(...) {
